...
class SumoEnv(MujocoEnv, EzPickle):
    ...
    def step(self, actions):  # ✅ Correctly overriding Gym's step()
        return self._step(actions)  # ✅ Calls the existing _step() function
    ...
    def _get_obs(self):
        if not self._mujoco_init:
            return np.concatenate([self.data.qpos.flat, self.data.qvel.flat])
        obs = tuple([agent.get_obs() for agent in self.agents])
        if obs is None or any(o is None for o in obs):
            raise ValueError("Observation returned None! Check agents' get_obs() methods.")
        return obs
