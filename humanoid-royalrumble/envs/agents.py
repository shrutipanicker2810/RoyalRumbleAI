# This is the base class for all agents. It is designed to work 
# with a MuJoCo environment (env) and uses an XML file to define the agent’s body.

# We set the action space and observation space here
...
class Agent(object):
    ...
    # Coefficients for reward/cost computation
    COST_COEFS = {
        'ctrl': 1e-1,
        # 'pain': 1e-4,
        # 'attack': 1e-1,
    ...
    JNT_NPOS = {
        0: 7,  # Free joint (position + quaternion)
        1: 4,  # Ball joint
        2: 1,  # Slide joint
        3: 1,  # Hinge joint
    }
    ...
    def __init__(self, env, scope, xml_path, adjust_z=0.):
        ...
        self._adjust_z = adjust_z # Optional z-axis offset for positioning
    ...
    def get_obs(self):
        try:
            ...
            obs_concat = np.concatenate(obs)

            if obs_concat is None or len(obs_concat) == 0:
                raise ValueError("get_obs() returned an empty or None observation!")

            return obs_concat
        except Exception as e:
            print(f"Error in get_obs(): {e}")
            return np.zeros(120)  # Return a placeholder instead of None
...
class Humanoid(Agent):
    ...
    def __init__(self, env, scope="humanoid", **kwargs):
        xml_path = os.path.join(os.path.dirname(__file__),
                                "assets", "humanoid.xml")
        super(Humanoid, self).__init__(env, scope, xml_path, **kwargs)
